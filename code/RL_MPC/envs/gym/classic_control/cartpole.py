"""
Modified to support continuous actions
"""
...
class ContinuousCartPoleEnv(gym.Env[np.ndarray, Union[int, np.ndarray]]):
    ...
    * v0: Initial versions release.
    ...
    def __init__(
        self, sutton_barto_reward: bool = False, render_mode: Optional[str] = None, reward_fn = "default"
    ):
    ...
    def step(self, action):
        ...
        # much simpler: just keep the pole upright
        if self.reward_fn == "height":
            reward = costheta

        if self.reward_fn == "l2":
            reward = -(1-costheta)**2 - theta_dot**2 

        if self.reward_fn == "linf":
            reward = -max([np.abs(1-costheta), np.abs(theta_dot)])
...
class CartPoleVectorEnv(VectorEnv):
    ...
    def __init__(
        ...
        self.single_action_space = spaces.Box(-self.force_mag, self.force_mag, dtype=np.float32)
...
if __name__ == '__main__':
    ...
    # gym.register('continuous-cartpole', 'cartpole:ContinuousCartPoleEnv')
    gym.register(
        id="ContinuousCartPole-v1",
        entry_point="cartpole:ContinuousCartPoleEnv",
        max_episode_steps=500,
        reward_threshold=475.0,
    )
    env = gym.make('ContinuousCartPole-v1')
